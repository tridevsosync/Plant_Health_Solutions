...
export const Route = createFileRoute("/cart")({
  head: () => ({
    meta: [
      { title: "Your Cart — Plant Health Solutions" },
      { name: "description", content: "Review your selected agricultural inputs, apply coupons and checkout." },
      { property: "og:title", content: "Your Cart — Plant Health Solutions" },
      { property: "og:description", content: "Review your selected agricultural inputs and checkout." },
    ],
  }),
  component: CartPage,
});
...
export function useCoupon() {
  ...
  return { code, setCode, applied, setApplied, coupon };
}
            ...
            product ? (
              <div key={line.id} className="flex flex-wrap items-center gap-4 rounded-xl border border-border bg-card p-4">
                ...
                <div className="min-w-[180px] flex-1">
                  <Link to="/products/$id" params={{ id: product.id }} className="font-semibold text-foreground hover:text-primary">
                    {product.name}
                  </Link>
